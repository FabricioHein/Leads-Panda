...
export default {
    init() {
        ...
        if (!val) {
            ...
            const list = store.state.app.countryList;
    ...
    toggleLanguage(item) {
        ...
        if (item) {
            ...
            let code = store.state.app.locale;
            if (!code) {
                code = localStorage.getItem('i18n_locale');
            }

            item = store.state.app.countryList.find((d) => d.code === code);
            if (item) {
                lang = item;
            }
        }
        ...
        if (!lang) {
            lang = store.state.app.countryList.find((d) => d.code === 'en');
        }
