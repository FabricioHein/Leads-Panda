                                ...
                                <div class="col-md-4 col-12">
                                    <form class="form-horizontal mt-md-0 mt-3 text-md-end text-center">
                                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#addEventsModal" @click="edit_event()">
                ...
                <div class="modal-dialog modal-md modal-dialog-centered">
                    <div class="modal-content mailbox-popup">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ params.id ? 'Editar Evento' : 'Add Evento' }}</h5>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group mb-4">
                                        <label for="start-date" class="">Título:</label>
                                ...
                                <div class="col-md-6 col-sm-6 col-12">
                                    <div class="form-group start-date mb-4">
                                        <label>De:</label>
                                        <flat-pickr v-model="params.start" :config="formatDay"
                                            class="form-control flatpickr active" placeholder="Start Date"></flat-pickr>
                                    </div>
                                </div>
                                <div class="col-md-6 col-sm-6 col-12">
                                    <div class="form-group end-date mb-4">
                                        <label for="end-date" class="">Até:</label>
                                        <flat-pickr v-model="params.end" :config="formatDay"
                                            class="form-control flatpickr active" placeholder="End Date"></flat-pickr>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-dismiss="modal"
                                data-bs-dismiss="modal" @click="cancel()">Cancelar</button>
                            <button type="button" class="btn btn-primary" @click="save_event()">{{ params.id ?
                                'Atualizar' : 'Add Evento' }}</button>
                        </div>
                    </div>
                </div>
            </div>
...
onMounted(  async ()  => {
   ...
   await bind_events();
    ...
    options.value.select = edit_date;
...
const bind_events = async () => {
    let dt = new Date();
    const todasAgendas = new AgendaService({}, token, `/api/agenda/todos/${usuario.id}`);
...
const cancel = async () => {
    ...
    if (params.value.id) {
        ...
        options.value.events = options.value.events.find((d) => d.id != addAgendaUsuario.id);
...
const save_event = async () => {
    ...
    if (params.value.id) {
        ...
        const addAgendaUsuario = new AgendaService(modelAgenda, token);
         ...
         if(data.id){
            const editEvento = new AgendaRetornoModel(data);
           
            options.value.events.push(editEvento);
            console.log(options.value.events)
